...
/**
 *   	\file       scrumtask_list.php
 *		\ingroup    scrumproject
 *		\brief      List page for scrumtask
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'scrumtasklist'; // To manage different context of search
...
$hookmanager->initHooks(array('scrumtasklist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->scrumproject->scrumtask->write;
$permissiontodelete = $user->rights->scrumproject->scrumtask->delete;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ScrumTask';
	$objectlabel = 'ScrumTask';
...
//$help_url="EN:Module_ScrumTask|FR:Module_ScrumTask_FR|ES:Módulo_ScrumTask";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("ScrumTasks"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/scrumproject/scrumtask_card.php', 1).'?id='.$id);
	exit;
}
...
$topicmail = "SendScrumTaskRef";
$modelmail = "scrumtask";
$objecttmp = new ScrumTask($db);
