...
/**
 *  \file       scrumkanban_contact.php
 *  \ingroup    scrumproject
 *  \brief      Tab for contacts linked to ScrumKanban
 */
...
$hookmanager->initHooks(array('scrumkanbancontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->scrumproject->scrumkanban->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('ScrumKanban')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("ScrumKanban"), -1, $object->picto);
