...
/**
 * \file    lib/scrumproject_scrumsprint.lib.php
 * \ingroup scrumproject
 * \brief   Library files with common functions for ScrumSprint
 */

/**
 * Prepare array of tabs for ScrumSprint
 *
 * @param	ScrumSprint	$object		ScrumSprint
 * @return 	array					Array of tabs
 */
function scrumsprintPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/scrumproject/scrumsprint_card.php", 1).'?id='.$object->id;
	...
	$upload_dir = $conf->scrumproject->dir_output."/scrumsprint/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/scrumproject/scrumsprint_document.php", 1).'?id='.$object->id;
	$head[$h][1] = $langs->trans('Documents');
	if (($nbFiles + $nbLinks) > 0) $head[$h][1] .= '<span class="badge marginleftonlyshort">'.($nbFiles + $nbLinks).'</span>';
	$head[$h][2] = 'document';
	$h++;

	$head[$h][0] = dol_buildpath("/scrumproject/scrumsprint_agenda.php", 1).'?id='.$object->id;
	...
	//	'entity:-tabname:Title:@scrumproject:/scrumproject/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'scrumsprint@scrumproject');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'scrumsprint@scrumproject', 'remove');
