...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'scrumsprintlistwizard'; // To manage different context of search
...
$hookmanager->initHooks(array('scrumsprintlistwizard')); // Note that conf->hooks_modules contains array
		...
		if(!empty($toselect)){
			...
			foreach ($toselect as $taskId){
				...
				if($task->fetch($taskId) > 0){
					...
					$scrumUserStory->qty =  ($task->planned_workload / 60 / 60) / $step  * $step;
					...
					if(empty($error)){
						...
						if($res > 0){
							...
							if($fk_scrum_sprint>0){
								...
								$scrumUserStorySprint->default_prod_calc = $scrumUserStory->default_prod_calc;
