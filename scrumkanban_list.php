...
/**
 *   	\file       scrumkanban_list.php
 *		\ingroup    scrumproject
 *		\brief      List page for scrumkanban
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'scrumkanbanlist'; // To manage different context of search
...
$hookmanager->initHooks(array('scrumkanbanlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->scrumproject->scrumkanban->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ScrumKanban';
	$objectlabel = 'ScrumKanban';
...
//$help_url = "EN:Module_ScrumKanban|FR:Module_ScrumKanban_FR|ES:Módulo_ScrumKanban";
$help_url = '';
$title = $langs->trans("ScrumKanbans");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/scrumproject/scrumkanban_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/scrumproject/scrumkanban_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendScrumKanbanRef";
$modelmail = "scrumkanban";
$objecttmp = new ScrumKanban($db);
