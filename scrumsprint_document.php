...
/**
 *  \file       scrumsprint_document.php
 *  \ingroup    scrumproject
 *  \brief      Tab for documents linked to ScrumSprint
 */
...
$hookmanager->initHooks(array('scrumsprintdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) $upload_dir = $conf->scrumproject->multidir_output[$object->entity ? $object->entity : $conf->entity]."/scrumsprint/".get_exdir(0, 0, 0, 1, $object);
...
$title = $langs->trans("ScrumSprint").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id)
{
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("ScrumSprint"), -1, $object->picto);
	...
	//$relativepathwithnofile='scrumsprint/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'scrumsprint/'.dol_sanitizeFileName($object->ref).'/';
