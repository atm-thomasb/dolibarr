...
/**
 * \file        class/scrumkanban.class.php
 * \ingroup     scrumproject
 * \brief       This file is a CRUD class file for ScrumKanban (Create/Read/Update/Delete)
 */
...
//	public $module = 'scrumproject'; // already included in $this->element
	 ...
	 */
	public $table_element = 'scrumproject_scrumkanban';
	...
	public $fields=array(
		...
		'background_url' => array('type'=>'varchar(500)', 'label'=>'KanbanBackgroundImageUrl', 'enabled'=>'1', 'position'=>60, 'notnull'=>0, 'visible'=>3, 'validate'=>'1',),
	...
	//  */
	// public $table_element_line = 'scrumproject_scrumkanbanline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_scrumkanban';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'ScrumKanbanline';
	...
	//  */
	// protected $childtablesoncascade = array('scrumproject_scrumkanbandet');

	// /**
	//  * @var ScrumKanbanLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->scrumproject->scrumkanban->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->scrumproject->scrumkanban->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->scrumproject->scrumkanban->scrumkanban_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SCRUMKANBAN_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'scrumkanban/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'scrumkanban/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->scrumproject->dir_output.'/scrumkanban/'.$oldref;
				$dirdest = $conf->scrumproject->dir_output.'/scrumkanban/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->scrumproject->dir_output.'/scrumkanban/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SCRUMKANBAN_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SCRUMKANBAN_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SCRUMKANBAN_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("ScrumKanban").'</u>';
		...
		$url = dol_buildpath('/scrumproject/scrumkanban_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowScrumKanban");
		...
		$hookmanager->initHooks(array('scrumkanbandao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new ScrumKanbanLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_scrumkanban = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->SCRUMPROJECT_SCRUMKANBAN_ADDON)) {
			$conf->global->SCRUMPROJECT_SCRUMKANBAN_ADDON = 'mod_scrumkanban_standard';
		}

		if (!empty($conf->global->SCRUMPROJECT_SCRUMKANBAN_ADDON)) {
			$mybool = false;

			$file = $conf->global->SCRUMPROJECT_SCRUMKANBAN_ADDON.".php";
			$classname = $conf->global->SCRUMPROJECT_SCRUMKANBAN_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_scrumkanban';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->SCRUMKANBAN_ADDON_PDF)) {
				$modele = $conf->global->SCRUMKANBAN_ADDON_PDF;
			}
		}
