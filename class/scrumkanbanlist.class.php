...
/**
 * \file        class/scrumkanbanlist.class.php
 * \ingroup     scrumproject
 * \brief       This file is a CRUD class file for ScrumKanbanList (Create/Read/Update/Delete)
 */
...
/**
 * Class for ScrumKanbanList
 */
class ScrumKanbanList extends CommonObject
{
	use CommonKanban;
	 ...
	 */
	public $table_element = 'scrumproject_scrumkanbanlist';
	...
	/**
	 * @var string String with name of icon for scrumkanbanlist. Must be the part after the 'object_' into object_scrumkanbanlist.png
	 */
	public $picto = 'scrumkanbanlist@scrumproject';
	...
	//  */
	// public $table_element_line = 'scrumproject_scrumkanbanlistline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_scrumkanbanlist';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'ScrumKanbanListline';
	...
	//  */
	// protected $childtablesoncascade = array('scrumproject_scrumkanbanlistdet');

	// /**
	//  * @var ScrumKanbanListLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->scrumproject->scrumkanbanlist->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->scrumproject->scrumkanbanlist->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->scrumproject->scrumkanbanlist->scrumkanbanlist_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SCRUMKANBANLIST_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'scrumkanbanlist/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'scrumkanbanlist/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->scrumproject->dir_output.'/scrumkanbanlist/'.$oldref;
				$dirdest = $conf->scrumproject->dir_output.'/scrumkanbanlist/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->scrumproject->dir_output.'/scrumkanbanlist/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SCRUMKANBANLIST_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SCRUMKANBANLIST_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SCRUMKANBANLIST_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("ScrumKanbanList").'</u>';
		...
		$url = dol_buildpath('/scrumproject/scrumkanbanlist_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowScrumKanbanList");
		...
		$hookmanager->initHooks(array('scrumkanbanlistdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new ScrumKanbanListLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_scrumkanbanlist = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->SCRUMPROJECT_SCRUMKANBANLIST_ADDON)) {
			$conf->global->SCRUMPROJECT_SCRUMKANBANLIST_ADDON = 'mod_scrumkanbanlist_standard';
		}

		if (!empty($conf->global->SCRUMPROJECT_SCRUMKANBANLIST_ADDON)) {
			$mybool = false;

			$file = $conf->global->SCRUMPROJECT_SCRUMKANBANLIST_ADDON.".php";
			$classname = $conf->global->SCRUMPROJECT_SCRUMKANBANLIST_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_scrumkanbanlist';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->SCRUMKANBANLIST_ADDON_PDF)) {
				$modele = $conf->global->SCRUMKANBANLIST_ADDON_PDF;
			}
		}
...
//		}
	...
	public function getKanBanListObjectFormatted(){
		...
		$object->title = '<div class="kanban-list-label-field">'.$this->label.'</div>'
			.'<div '
				.' id="'.$object->id.'-dropdown-menu" '
				.' data-listid="'.$this->id.'" '
				.' class="kanban-header-dropdown-btn" '
...
///**
// * Class ScrumKanbanListLine. You can also remove this and generate a CRUD class for lines objects.
// */
//class ScrumKanbanListLine extends CommonObjectLine
//{
//	// To complete with content of an object ScrumKanbanListLine
//	// We should have a field rowid, fk_scrumkanbanlist and position
//
//	/**
//	 * @var int  Does object support extrafields ? 0=No, 1=Yes
//	 */
